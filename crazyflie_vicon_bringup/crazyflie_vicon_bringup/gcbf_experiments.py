...
LAND_DURATION = 3.0
HOVER_TIME = 3.0
POSITION_EXCHANGE_DURATION = 20.0
RATE_HZ = 50.0
LOOKAHEAD_DT = 0.05
CIRCLE_RADIUS = 1.0
...
def log_status(swarm, cf, label):
    ...
    swarm.allcfs.get_logger().info(
        ...
        f'rx_unicast={status.get("num_rx_unicast", 0)} '
        f'tx_unicast={status.get("num_tx_unicast", 0)}'
    )


def status_has_preflight_fields(status):
    return all(
        key in status
        for key in ('supervisor', 'battery', 'pm_state', 'rssi')
    )


def wait_for_status(swarm, cf, time_helper, timeout=STATUS_WAIT_TIMEOUT):
    if swarm.allcfs.get_parameter('use_sim_time').value:
        return cf.get_status()

    start_time = time_helper.time()
    last_log_time = start_time - 1.0
    while time_helper.time() - start_time < timeout:
        status = cf.get_status()
        if status_has_preflight_fields(status):
            return status

        now = time_helper.time()
        if now - last_log_time >= 1.0:
            swarm.allcfs.get_logger().info(
                f'Waiting for first status sample from {cf.prefix}.'
            )
            last_log_time = now
        time_helper.sleep(0.05)

    raise RuntimeError(
        f'Timed out waiting for status from {cf.prefix}. '
        'Check that crazyflie_server is running and the radio link is connected.'
    )
...
def require_can_fly(swarm, cf):
    ...
    rssi = int(status.get('rssi', 0))
    if not supervisor & 8 or battery <= 0.0:
        raise RuntimeError(
            f'{cf.prefix} is not ready to fly: supervisor={supervisor}, battery={battery:.2f}V.'
        )
    if rssi < MIN_RADIO_RSSI:
        raise RuntimeError(f'{cf.prefix} radio RSSI too low: rssi={rssi}, minimum={MIN_RADIO_RSSI}.')
...
def stale_pose_details(cfs, now):
    stale = []
    for cf in cfs:
        stamp = cf.poseStamped
        if not stamp:
            stale.append(f'{cf.prefix.lstrip("/")}: no pose sample')
            continue
        msg_time = float(stamp['timestamp_sec']) + float(stamp['timestamp_nsec']) * 1e-9
        if msg_time > 0.0 and now - msg_time > POSE_STALE_SECONDS:
            stale.append(f'{cf.prefix.lstrip("/")}: age={now - msg_time:.2f}s')
    return stale


def poses_are_stale(cfs, now):
    return bool(stale_pose_details(cfs, now))


def stale_pose_message(cfs, now):
    return ', '.join(stale_pose_details(cfs, now))
...
def stream_takeoff_all(cfs, time_helper, swarm, mocap_positions):
    logger = swarm.allcfs.get_logger()
    wait_for_poses(cfs, time_helper)
    starts = current_positions(cfs)
    targets = starts.copy()
    ...
    logger.info(
        'Starting cmdFullState streamed takeoff for: '
        + ', '.join(cf.prefix for cf in cfs)
    )
    for cf in cfs:
        cf.arm(True)
    time_helper.sleep(0.2)
    ...
    last_log_time = start_time - 1.0
    settled_start = None
    first_setpoint_logged = False

    while time_helper.time() - start_time < TAKEOFF_WAIT_TIMEOUT:
        now = time_helper.time()
        elapsed = now - start_time
        ramp = np.clip(elapsed / TAKEOFF_DURATION, 0.0, 1.0)
        alpha = ramp * ramp * (3.0 - 2.0 * ramp)
        alpha_dot = 6.0 * ramp * (1.0 - ramp) / TAKEOFF_DURATION
        alpha_ddot = 6.0 * (1.0 - 2.0 * ramp) / (TAKEOFF_DURATION**2)

        refs = starts + alpha * (targets - starts)
        vel_refs = alpha_dot * (targets - starts)
        acc_refs = alpha_ddot * (targets - starts)

        stale = stale_pose_message(cfs, now)
        if stale:
            raise RuntimeError(f'Pose data became stale during streamed takeoff: {stale}.')
        for cf in cfs:
            if is_tumbled(cf):
                cf.emergency()
                raise RuntimeError(f'{cf.prefix} tumbled during streamed takeoff.')
        for cf in cfs:
            z = measured_height(swarm, cf, mocap_positions)
            if z is not None and z > max_z:
                cf.emergency()
                raise RuntimeError(f'{cf.prefix} exceeded streamed takeoff altitude limit.')
        if len(cfs) > 1 and pairwise_too_close(current_positions(cfs), SAFE_RADIUS):
            raise RuntimeError('Pairwise distance dropped below safe radius during streamed takeoff.')

        for cf, ref, vel_ref, acc_ref in zip(cfs, refs, vel_refs, acc_refs):
            cf.cmdFullState(ref, vel_ref, acc_ref, 0.0, np.zeros(3))

        if not first_setpoint_logged:
            for cf, ref, vel_ref, acc_ref in zip(cfs, refs, vel_refs, acc_refs):
                estimator = cf.get_position()
                z = measured_height(swarm, cf, mocap_positions)
                logger.info(
                    f'First streamed takeoff cmdFullState for {cf.prefix}: '
                    f'estimator_pos=[{estimator[0]:+.3f}, {estimator[1]:+.3f}, {estimator[2]:+.3f}], '
                    f'mocap_z={z}, '
                    f'pos_ref=[{ref[0]:+.3f}, {ref[1]:+.3f}, {ref[2]:+.3f}], '
                    f'vel_ref=[{vel_ref[0]:+.3f}, {vel_ref[1]:+.3f}, {vel_ref[2]:+.3f}], '
                    f'acc=[{acc_ref[0]:+.3f}, {acc_ref[1]:+.3f}, {acc_ref[2]:+.3f}]'
                )
            first_setpoint_logged = True
        ...
        if now - last_log_time >= 1.0:
            ...
            for cf, ref in zip(cfs, refs):
                ...
                status = cf.get_status()
                z = measured_height(swarm, cf, mocap_positions)
                if z is not None and abs(z - HEIGHT) <= TAKEOFF_HEIGHT_TOLERANCE:
                    reached.append(cf.prefix.lstrip('/'))
                else:
                    waiting.append(cf.prefix.lstrip('/'))
                logger.info(
                    f'streamed takeoff monitor {cf.prefix}: '
                    f'estimator_pos=[{estimator[0]:+.3f}, {estimator[1]:+.3f}, {estimator[2]:+.3f}], '
                    f'mocap_z={z}, '
            ...
            logger.info(
                'streamed takeoff readiness: '
                f'reached=[{", ".join(reached)}] waiting=[{", ".join(waiting)}]'
            )
            last_log_time = now

        heights = [
            measured_height(swarm, cf, mocap_positions)
            for cf in cfs
        ]
        all_reached = all(
            z is not None and abs(z - HEIGHT) <= TAKEOFF_HEIGHT_TOLERANCE
            for z in heights
        )
        if all_reached and elapsed >= TAKEOFF_DURATION:
            if settled_start is None:
                settled_start = now
            if now - settled_start >= TAKEOFF_SETTLE_TIME:
                logger.info('All Crazyflies reached streamed takeoff height; continuing cmdFullState hover.')
                return
        else:
            settled_start = None
    ...
    raise RuntimeError('Not all Crazyflies reached streamed takeoff height before timeout.')


def stream_circular_position_exchange(cfs, time_helper, swarm, mocap_positions, duration=POSITION_EXCHANGE_DURATION):
    logger = swarm.allcfs.get_logger()
    wait_for_poses(cfs, time_helper)
    starts = current_positions(cfs)
    radii = np.linalg.norm(starts[:, :2], axis=1)
    start_angles = np.arctan2(starts[:, 1], starts[:, 0])
    target_angles = start_angles + np.pi
    max_z = HEIGHT + DEBUG_MAX_EXTRA_HEIGHT

    logger.info(
        'Starting cmdFullState circular position exchange for: '
        + ', '.join(cf.prefix for cf in cfs)
    )
    for cf, start, radius, target_angle in zip(cfs, starts, radii, target_angles):
        target = np.array(
            [
        ...
        logger.info(
            f'{cf.prefix} circular exchange target: '
    ...
    while time_helper.time() - start_time < duration:
        now = time_helper.time()
        t = np.clip((now - start_time) / duration, 0.0, 1.0)
        smooth = t * t * (3.0 - 2.0 * t)
        smooth_dot = 6.0 * t * (1.0 - t) / duration
        smooth_ddot = 6.0 * (1.0 - 2.0 * t) / (duration**2)
        theta = start_angles + np.pi * smooth
        theta_dot = np.pi * smooth_dot
        theta_ddot = np.pi * smooth_ddot

        refs = np.zeros_like(starts)
        vel_refs = np.zeros_like(starts)
        acc_refs = np.zeros_like(starts)
        refs[:, 0] = radii * np.cos(theta)
        refs[:, 1] = radii * np.sin(theta)
        refs[:, 2] = HEIGHT
        vel_refs[:, 0] = -radii * np.sin(theta) * theta_dot
        vel_refs[:, 1] = radii * np.cos(theta) * theta_dot
        acc_refs[:, 0] = -radii * (
            np.cos(theta) * theta_dot**2 + np.sin(theta) * theta_ddot
        )
        acc_refs[:, 1] = radii * (
        ...
        if stale:
            raise RuntimeError(
                f'Pose data became stale during circular position exchange: {stale}.'
            )
        for cf in cfs:
            if is_tumbled(cf):
                cf.emergency()
                raise RuntimeError(f'{cf.prefix} tumbled during circular position exchange.')
        for cf in cfs:
            z = measured_height(swarm, cf, mocap_positions)
            if z is not None and z > max_z:
                cf.emergency()
                raise RuntimeError(f'{cf.prefix} exceeded circular exchange altitude limit.')
        if len(cfs) > 1 and pairwise_too_close(current_positions(cfs), SAFE_RADIUS):
            raise RuntimeError('Pairwise distance dropped below safe radius during circular position exchange.')
        ...
        if now - last_log_time >= 1.0:
            ...
            for cf, position, ref, vel_ref, acc_ref in zip(cfs, positions, refs, vel_refs, acc_refs):
                z = measured_height(swarm, cf, mocap_positions)
                logger.info(
                    f'circular exchange monitor {cf.prefix}: '
                    f'estimator_pos=[{position[0]:+.3f}, {position[1]:+.3f}, {position[2]:+.3f}], '
                    f'mocap_z={z}, '
                    f'path_alpha={smooth:.2f}, '
                    f'pos_ref=[{ref[0]:+.3f}, {ref[1]:+.3f}, {ref[2]:+.3f}], '
                    f'vel_ref=[{vel_ref[0]:+.3f}, {vel_ref[1]:+.3f}, {vel_ref[2]:+.3f}], '
                    f'acc=[{acc_ref[0]:+.3f}, {acc_ref[1]:+.3f}, {acc_ref[2]:+.3f}]'
                )
    ...
    final_refs = starts.copy()
    final_refs[:, 0] = -starts[:, 0]
    final_refs[:, 1] = -starts[:, 1]
    final_refs[:, 2] = HEIGHT
    final_start = time_helper.time()
    logger.info(f'Holding circular exchange endpoint for {HOVER_TIME:.1f} s before landing.')
    while time_helper.time() - final_start < HOVER_TIME:
        for cf, ref in zip(cfs, final_refs):
            cf.cmdFullState(ref, np.zeros(3), np.zeros(3), 0.0, np.zeros(3))
        time_helper.sleepForRate(RATE_HZ)
...
def stream_setpoints(
    ...
    while time_helper.time() - start < max_time:
        ...
        stale = stale_pose_message(cfs, now)
        if stale:
            raise RuntimeError(f'Pose data became stale: {stale}.')
        for cf in cfs:
            if is_tumbled(cf):
                cf.emergency()
