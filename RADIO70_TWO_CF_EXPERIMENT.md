...
one Crazyradio. Both links use radio index `0`, channel `70`, and 2 Mbit/s.
...
| --- | --- | --- |
| `radio70_single.launch.py` | `cf8` (`E7E7E7E708`) | Rehearse both sequential flight slots with one physical Crazyflie |
| `radio70_dual.launch.py` | `cf8`, `cf9` (`E7E7E7E708`, `E7E7E7E709`) | Test two physical Crazyflies through Crazyradio `0` |

Before testing, use CFclient to set each physical Crazyflie to channel `70`,
...
ros2 launch crazyflie_vicon_bringup radio70_single.launch.py
```
...
source /opt/ros/humble/setup.bash
source /home/minhyuk/GCBF_EXPERIMENTS_WS/install/local_setup.bash
ros2 run crazyflie_vicon_bringup radio70_sequential
```
...
Place `cf8` at `[0.0, -1.0, 0.0]` and `cf9` at `[1.0, 0.0, 0.0]`.

Start `radio70_dual.launch.py` in Terminal 1, using the same environment setup:

```bash
ros2 launch crazyflie_vicon_bringup radio70_dual.launch.py
```
